...
void adc_ISR(void);
# 7 "interrupt_manager.c" 2

u8 volatile RB4_Flag=1,RB5_Flag=1;
# 46 "interrupt_manager.c"
void __attribute__((picinterrupt(("")))) manage(void)
{
    if((1==INTCONbits.INT0IF)&&(1==INTCONbits.INT0IE))
    {
        INT0_ISR();
    }
    if((1==INTCON3bits.INT1IF)&&(1==INTCON3bits.INT1IE))
    {
        INT1_ISR();
    }
    if((1==INTCON3bits.INT2IF)&&(1==INTCON3bits.INT2IE))
    {
        INT2_ISR();
    }
    ...
    if((1==PIR1bits.TMR1IF)&&(1==PIE1bits.TMR1IE))
    {
        Timer1_ISR();
    }
    if((1==PIR1bits.TMR2IF)&&(1==PIE1bits.TMR2IE))
    {
        Timer2_ISR();
    }
    if((1==PIR2bits.TMR3IF)&&(1==PIE2bits.TMR3IE))
    {
        timer3_ISR();
    }
    if((1==PIR1bits.ADIF)&&(1==PIE1bits.ADIE))
    {
        adc_ISR();
    }
